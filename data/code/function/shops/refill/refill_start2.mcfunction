$execute as @e[type=chest_boat,tag=shop_marker,tag=shop$(id),limit=1, tag=!shop.checked] run function code:shops/refill/refill with storage sandmines:macros shop
$execute unless score $amount_shops shop.data matches -2147483648..2147483647 store result score $amount_shops shop.data if entity @e[type=chest_boat,tag=shop$(id)]
